...
#include <linux/wakelock.h>	/*zte doumm modify*/
/*zte doumm add for soc sync*/
...
/*zte doumm add for self-adaption charger feature start*/
...
/*zte doumm add for self-adaption charger feature end*/
/*zte doumm add for usb plug in/out wakelock start*/
...
struct smb1360_chip {
	...
	struct delayed_work		delayed_testa_work;	/*zte doumm add for self-adaption charger feature*/
	...
	int				soft_hot_thresh;
	/*zte doumm add for soc sync start*/
	#if defined FEATURE_SYNC_SOC
	int				ui_soc;
	int				fg_soc;
	bool			charging_done_status;
	#endif
	/*zte doumm add for soc sync end*/
	int chg_max_current;	/*zte add chg current*/
...
static enum power_supply_property smb1360_battery_properties[] = {
	...
	POWER_SUPPLY_PROP_TECHNOLOGY,/*ZTE_MODIFY doumm for battery tech*/
};
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL:
		...
		break;
	/*ZTE MODIFY doumm add for battery tech ++*/
...
static void smb1360_external_power_changed(struct power_supply *psy)
{
	...
	/*zte doumm add for self-detection charger feature*/
...
static int usbin_uv_handler(struct smb1360_chip *chip, u8 rt_stat)
{
	...
	if (chip->usb_present && !usb_present) {
		...
		/*if (probe_done_flag) {*/
		/*zte doumm add for wake lock when plug in/out*/
			wake_lock_timeout(&usb_inout_irq_wakelock, 9*HZ);
		/*}*/
		testa_flag = 1;
		cancel_delayed_work_sync(&chip->delayed_testa_work);	/*zte doumm add for cancel testa work*/
...
/*zte doumm add for battery profile select start*/
	...
	#endif
/*zte doumm add for battery profile select end*/
...
static int smb1360_hw_init(struct smb1360_chip *chip)
{
	...
	if (chip->client->irq) {
		mask = CHG_STAT_IRQ_ONLY_BIT
			...
			| CHG_STAT_LOW_BAT_BLINK_BIT;	/*ZTEMODIFY doumm add for this pin is not used*/
