...
**
** $Id: si3217x_intf.c 5180 2015-10-15 14:53:17Z nizajerk $
**
** SI3217X_Intf.c
...
#define fmt_string "%sChannel %d : Type = %s Rev = %d\n"
#else
          const char fmt_string[] = "%sChannel %d : Type = %s Rev = %d\n";
#endif
        ...
        LOGPRINT(fmt_string, LOGPRINT_PREFIX, pProslic[k]->channel, dev_type, pProslic[k]->deviceId->chipRev);
      }
#endif /* ENABLE_DEBUG */
     if(pProslic[k]->channelType == PROSLIC)
     {
  ...
  if(device_count == 0)
  {
    ...
    if(pProslic[0]->debugMode)
      LOGPRINT("%s: failed to detect any ProSLICs", LOGPRINT_PREFIX);
#endif
    return RC_SPI_FAIL;
  }
