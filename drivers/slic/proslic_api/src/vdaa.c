...
**
** $Id: vdaa.c 5120 2015-10-01 00:46:58Z nizajerk $
**
...
static int isVerifiedDAA(vdaaChanType *pVdaa){
	...
	if ( (data & 0x40) == 0 ) {/*This bit is always 1 for DAA*/
            LOGPRINT("DAA device not detected");
...
#ifdef ENABLE_DEBUG
    LOGPRINT("DAA : ChipRev = 0x%x, LSType = 0x%x", pVdaa->deviceId->chipRev,pVdaa->deviceId->lsType);
#endif
  }
...
int Vdaa_Init (vdaaChanType_ptr *pVdaa,int size){
    ...
    if(pVdaa[k]->channelType == PROSLIC) continue;  /* Skip if we know this is a ProSLIC, else check the device out */
...
#ifdef ENABLE_DEBUG
           if (pVdaa[k]->debugMode)
              LOGPRINT("No DAA devices detected");
#endif
#endif
           return RC_SPI_FAIL;
        }
...
#ifdef ENABLE_DEBUG
        if (pVdaa[k]->debugMode)
            LOGPRINT("DAA initialization completed");
#endif
	return RC_NONE;
}
