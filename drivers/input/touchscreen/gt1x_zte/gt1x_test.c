...
int parse_config(unsigned char *config)
{
	...
	if (!strncmp((const char *)gt1x_version.product_id, "1143", 4)) {
		...
		sys.sc_driver_num = sys.driver_num -
							(config[REG_MODULE_SWITCH_DK - REG_CONFIG_BASE] & 0x01);

		sys.sc_sensor_num = sys.sensor_num;

		sys.key_number = (config[46] & 0x1F) + (config[47] & 0x1F) +
						(config[48] & 0xFF) - sys.sc_driver_num * sys.sc_sensor_num;
		...
		if (config[REG_MODULE_SWITCH_DK - REG_CONFIG_BASE] & 0x01) {
			...
			sys.sc_sensor_num = sys.sensor_num -
				(config[REG_MODULE_SWITCH_SK - REG_CONFIG_BASE] & 0x01);
...
int check_version(unsigned char *ini_version)
{
	...
	snprintf((char *)tmp, 128, "%d.%d.%d_%d.%d", (gt1x_version.mask_id >> 16) &
			0x0FF, (gt1x_version.mask_id >> 8) & 0x0FF, (gt1x_version.mask_id & 0xFF),
	...
	for (i = 0; i < short_status[0]; i++) {
		...
		for (j = i + 1; j < sys.max_driver_num; j++) {
			...
			if (short_code > gt900_short_threshold) {
				...
				test_error_code |= _calculate_short_resistance(r_data,
								gt900_drv_drv_resistor_threshold, 0);
	...
	for (i = 0; i < short_status[1]; i++) {
		...
		for (j = 0; j < sys.max_sensor_num; j++) {
			...
			if (j == i || (j < i && (j & 0x01) == 0))
				...
				continue;
			/* The first one have no last pin.*/
			...
			if (short_code > gt900_short_threshold) {
				...
				test_error_code |= _calculate_short_resistance(r_data,
								gt900_sen_sen_resistor_threshold, 0);
		...
		/*Tx-RX*/
	 {
		...
		for (j = 0; j < sys.max_sensor_num; j++) {
			...
			if (short_code > gt900_short_threshold) {
				...
				test_error_code |= _calculate_short_resistance(r_data,
								gt900_drv_sen_resistor_threshold, 0);
...
int auto_find_ini(char *findDir, char *format, char *inipath)
{
	...
	snprintf((char *)tmp, 64, "/system/etc/gtp_test_sensor_%d0.ini", gt1x_version.sensor_id);
...
int ini_read(char *file, const char *key, char *value)
{
	...
	while (1) {
		...
		if (strcmp((const char *)buf, (const char *)key) == 0) {
...
static ssize_t gtp_sysfs_openshort_show(struct device *dev, struct device_attribute *attr, char *buff)
{
	...
	pr_info("stone gtp_sysfs_openshort_show ,\n");

	ret = open_short_test(result);

	if (ret < 0) {

		len += snprintf((char *)&buff[len], 64, "test error code 0x%X", (unsigned int)ret);

	}

	else if (ret == 0) {

		len += snprintf((char *)&buff[len], 64, "PASS");

	}

	else {

		len += snprintf((char *)&buff[len], 64, "NG\r\n");

		if (ret & _BEYOND_MAX_LIMIT) {

			len += snprintf((char *)&buff[len], 64, "beyond max limit\r\n");

		}

		if (ret & _BEYOND_MIN_LIMIT) {

			len += snprintf((char *)&buff[len], 64, "beyond min limit\r\n");

		}

		if (ret & _BEYOND_ACCORD_LIMIT) {

			len += snprintf((char *)&buff[len], 64, "beyond accord limit\r\n");

		}

		if (ret & _BETWEEN_ACCORD_AND_LINE) {
			len += (s32) snprintf((char *)&buff[len], 64, "accord limit fuzzy\r\n");
		}

		if (ret & _BEYOND_OFFSET_LIMIT) {

			len += snprintf((char *)&buff[len], 64, "beyond full offset limit\r\n");

		}

		if (ret & _BEYOND_JITTER_LIMIT) {

			len += snprintf((char *)&buff[len], 64, "beyond full jitter limit\r\n");

		}

		if (ret & _KEY_BEYOND_MAX_LIMIT) {

			len += snprintf((char *)&buff[len], 64, "beyond key max limit\r\n");

		}

		if (ret & _KEY_BEYOND_MIN_LIMIT) {

			len += snprintf((char *)&buff[len], 64, "beyond key min limit\r\n");

		}

		if (ret & _BEYOND_UNIFORMITY_LIMIT) {

			len += snprintf((char *)&buff[len], 64, "beyond uniformity limit\r\n");

		}

		if (ret & _VERSION_ERR) {

			len += snprintf((char *)&buff[len], 64, "fw version error\r\n");

		}

		if (ret & _MODULE_TYPE_ERR) {

			len += snprintf((char *)&buff[len], 64, "sensor id not match\r\n");

		}

		if (ret & _GT_SHORT) {

			len += snprintf((char *)&buff[len], 64, "short circuit\r\n");

			for (i = 0; i < result[0]; i++) {

				if (result[1 + i * 4] & 0x80) {

					len += snprintf((char *)&buff[len], 64, "drv%d & ", result[1 + i * 4] & 0x7F);

				}

				else if (result[1 + i * 4] == (sys.max_driver_num + 1)) {

					len += snprintf((char *)&buff[len], 64, "GND\\VDD & ");

				}

				else {

					len += snprintf((char *)&buff[len], 64, "sen%d & ", result[1 + i * 4] & 0x7F);

				}
				if (result[2 + i * 4] & 0x80) {

					len += snprintf((char *)&buff[len], 64, "drv%d", result[2 + i * 4] & 0x7F);
					...
					len += snprintf((char *)&buff[len], 64, "sen%d", result[2 + i * 4] & 0x7F);

				}
				len += snprintf((char *)&buff[len], 64, "(%dk)\r\n",
...
static ssize_t gtp_sysfs_openshort_store(struct device *dev, struct device_attribute *attr,
											const char *buff, size_t count)
{
	...
	pr_info("stone gtp_sysfs_openshort_store ,\n");
	...
	for (i = 0; i < 100; i++) {
		...
		bytes += snprintf(&buf[bytes], 64, "%d,", i);
