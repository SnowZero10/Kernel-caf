...
#include <linux/rtc.h>
#define DEBUG(fmt, arg...) pr_info("<<GTP-TEST-DBG>>[%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
#define WARNING(fmt, arg...) pr_info("<<GTP-TEST-WAR>>[%s:%d] "fmt"\n", __func__, __LINE__, ##arg)
...
#define FORMAT_PATH(path, mdir, name) do {\
			       ...
			       snprintf((char *)path, 128, "%s%s_%04d%02d%02d%02d%02d%02d.csv",\
			       mdir, name, (tm.tm_year+1900), (tm.tm_mon + 1),\
...
#define FORMAT_PATH(path, mdir, name) do {\
			struct timeb tp;\
		      ...
		      snprintf((char *)path, 128, "%s%s_%04d%02d%02d%02d%02d%02d.csv",\
		      mdir, name, (1990 + tm->tm_year), 1+tm->tm_mon,\
		      tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec);\
		     } while (0)
