...
static void _get_channel_average_value(void)
{
	...
	if (current_data_index == 0) {
		memset(channel_average, 0, sys.sc_sensor_num * sys.sc_driver_num);
		memset(channel_square_average, 0, sys.sc_sensor_num * sys.sc_driver_num);
	}
...
static unsigned char _check_key_min_value(void)
{
	...
	for (i = sys.sc_sensor_num * sys.sc_driver_num, j = 0;
		i < sys.sc_sensor_num * sys.sc_driver_num + sys.key_number; i++) {
		...
		for (j = 0; j < sys.sc_driver_num; j++) {
			...
			if (j == 0) {
				if (need_check[i + (j + 1) * sys.sc_sensor_num] != _NEED_NOT_CHECK) {
					accord_temp =
					    (FLOAT_AMPLIFIER *
					    abs((s16) (current_data_temp[i + (j + 1) *
					    sys.sc_sensor_num] - current_data_temp[index]))) /
			...
			if (i == 0) {
				if (need_check[i + 1 + j * sys.sc_sensor_num] != _NEED_NOT_CHECK) {
					temp =
					    (FLOAT_AMPLIFIER *
					    abs((s16) (current_data_temp[i + 1 + j *
					    sys.sc_sensor_num] - current_data_temp[index]))) /
					    current_data_temp[index];
				}
				if (temp > accord_temp) {
...
/*                              current_data_temp[index] = 1;*/
			...
			if (i == 0) {
				if ((need_check[i + 1 + j * sys.sc_sensor_num] != _NEED_NOT_CHECK) &&
					(need_check[i + (j + sign) * sys.sc_sensor_num] != _NEED_NOT_CHECK)
					...
					accord_temp =
						...
						sys.sc_sensor_num] - current_data_temp[index])) +
					     FLOAT_AMPLIFIER *
					     abs((s16) (current_data_temp[i + 1 + (j + sign) *
					     sys.sc_sensor_num] - current_data_temp[i + (j + sign) *
					     sys.sc_sensor_num]))) /
					    current_data_temp[index];
				}
			} else if (i == sys.sc_sensor_num - 1) {
				...
				if ((need_check[i + 1 + j * sys.sc_sensor_num] != _NEED_NOT_CHECK) &&
					(need_check[i - 1 + j * sys.sc_sensor_num] != _NEED_NOT_CHECK)) {
					accord_temp =
					    (FLOAT_AMPLIFIER *
						abs((s16) (current_data_temp[i + 1 + j *
						sys.sc_sensor_num] - current_data_temp[index])) +
...
static unsigned char _check_full_screen_offest(unsigned char special_check)
{
	...
	for (i = 0; i < sys.sc_sensor_num * sys.sc_driver_num; i++) {
		...
		offset_temp = abs((current_data_temp[i] - average_temp) *
					FLOAT_AMPLIFIER) / average_temp;
...
/*
		...
		if (offset_temp > offset_limit) {
			if ((special_check == _SPECIAL_CHECK) &&
				(special_num < _SPECIAL_LIMIT_CHANNEL_NUM) &&
				(offset_temp <= special_limit)) {
...
static unsigned char _rawdata_test_result_analysis(int check_types)
{
	...
	if (((check_types & _KEY_MAX_CHECK) != 0) && ((test_error_code & _KEY_BEYOND_MAX_LIMIT) != 0)) {
		for (i = sys.sc_sensor_num * sys.sc_driver_num;
			i < sys.sc_sensor_num * sys.sc_driver_num + sys.key_number; i++) {
		...
		if ((check_types & _FAST_TEST_MODE) != 0) {
			if (samping_set_number < samping_num) {
				...
				for (i = 0; i < sys.sc_sensor_num * sys.sc_driver_num; i++) {
					...
					channel_square_average[i] += channel_square_average[i] *
					temp / samping_set_number;
				}
				return 0;
			}
...
static s32 _save_testing_data(char *save_test_data_dir, int test_types)
{
	...
	if (current_data_index == 0) {
		bytes = (s32) snprintf((char *)data, TEST_TEMP_LENGTH, "Config:\n");
		for (i = 0; i < sys.config_length; i++) {
			bytes += (s32) snprintf((char *)&data[bytes],
				TEST_TEMP_LENGTH, "0x%02X,", module_cfg[i]);
		}
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "\n\n");
		...
		if ((test_types & _MAX_CHECK) != 0) {
			bytes = (s32) snprintf((char *)data, TEST_TEMP_LENGTH, "Channel maximum:\n");
			for (i = 0; i < sys.sc_sensor_num; i++) {
				for (j = 0; j < sys.sc_driver_num; j++) {
					bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH,
								"%d,", max_limit_value[i + j * sys.sc_sensor_num]);
				}
				bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "\n");
				ret = fwrite(data, bytes, 1, fp);
				bytes = 0;
			...
			for (i = 0; i < sys.key_number; i++) {
				if (_node_in_key_chn(i + sys.sc_sensor_num *
					sys.sc_driver_num, module_cfg, sys.key_offest) < 0) {
					continue;
				}
				bytes += (s32) snprintf((char *)&data[bytes],
					TEST_TEMP_LENGTH, "%d,", max_key_limit_value[j++]);
		...
		if ((test_types & _MIN_CHECK) != 0) {
			bytes = (s32) snprintf((char *)data, TEST_TEMP_LENGTH, "Channel minimum:\n");
			for (i = 0; i < sys.sc_sensor_num; i++) {
				for (j = 0; j < sys.sc_driver_num; j++) {
					bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH,
					"%d,", min_limit_value[i + j * sys.sc_sensor_num]);
				}
				bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "\n");
				ret = fwrite(data, bytes, 1, fp);
				bytes = 0;
			...
			for (i = 0; i < sys.key_number; i++) {
				...
				bytes += (s32) snprintf((char *)&data[bytes],
						TEST_TEMP_LENGTH, "%d,", min_key_limit_value[j++]);
			}
			bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "\n");
			ret = fwrite(data, bytes, 1, fp);
			if (ret < 0) {
		...
		if ((test_types & _ACCORD_CHECK) != 0) {
			bytes = (s32) snprintf((char *)data, TEST_TEMP_LENGTH,
					"Channel average:(%d)\n", FLOAT_AMPLIFIER);
			for (i = 0; i < sys.sc_sensor_num; i++) {
				for (j = 0; j < sys.sc_driver_num; j++) {
					bytes += (s32) snprintf((char *)&data[bytes],
						TEST_TEMP_LENGTH, "%ld,", accord_limit[i + j * sys.sc_sensor_num]);
				}
				bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "\n");
				ret = fwrite(data, bytes, 1, fp);
				bytes = 0;
			...
			if (sys.chip_type == _GT1143) {
				bytes = (s32) snprintf((char *)data, TEST_TEMP_LENGTH,
							"Channel line accord:(%d)\n", FLOAT_AMPLIFIER);
				for (i = 0; i < sys.sc_sensor_num; i++) {
					for (j = 0; j < sys.sc_driver_num; j++) {
						bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH,
								"%ld,", accord_line_limit[i + j * sys.sc_sensor_num]);
					}
					bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "\n");
					ret = fwrite(data, bytes, 1, fp);
					bytes = 0;
		...
		bytes = (s32) snprintf((char *)data, TEST_TEMP_LENGTH, " Rawdata\n");
		ret = fwrite(data, bytes, 1, fp);
		if (ret < 0) {
			WARNING("write to file fail.");
			goto exit_save_testing_data;
	...
	bytes = (s32) snprintf((char *)data, TEST_TEMP_LENGTH, "No.%d\n", current_data_index);
	ret = fwrite(data, bytes, 1, fp);
	if (ret < 0) {
		WARNING("write to file fail.");
		goto exit_save_testing_data;
	...
	for (i = 0; i < sys.sc_sensor_num; i++) {
		...
		for (j = 0; j < sys.sc_driver_num; j++) {
			...
			bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "%d,", tmp);
			if (tmp > max) {
				max = tmp;
			}
			if (tmp < min) {
	...
	if (sys.key_number > 0) {
		bytes = (s32) snprintf((char *)data, TEST_TEMP_LENGTH, "Key Rawdata:\n");
		for (i = 0; i < sys.key_number; i++) {
			if (_node_in_key_chn(i + sys.sc_sensor_num *
				sys.sc_driver_num, module_cfg, sys.key_offest) < 0) {
				continue;
			...
			bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH,
						"%d,", current_data_temp[i + sys.sc_sensor_num * sys.sc_driver_num]);
		}
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "\n");
		ret = fwrite(data, bytes, 1, fp);
		if (ret < 0) {
	...
	if ((test_types & _ACCORD_CHECK) != 0) {
		...
		for (i = 0; i < sys.sc_sensor_num; i++) {
			...
			for (j = 0; j < sys.sc_driver_num; j++) {
				bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH,
						"%ld,", channel_max_accord[i + j * sys.sc_sensor_num]);
			}
			bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "\n");
			ret = fwrite(data, bytes, 1, fp);
			if (ret < 0) {
...
static s32 _save_test_result_data(char *save_test_data_dir, int test_types, u8 *shortresult)
{
	...
	bytes = (s32) snprintf((char *)data, TEST_TEMP_LENGTH, "Test Result:");
	if (test_error_code == _CHANNEL_PASS) {
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Pass\n\n");
	} else {
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Fail\n\n");
	}
	bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Test items:\n");
	if ((test_types & _MAX_CHECK) != 0) {
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Max Rawdata:  ");
		if (test_error_code & _BEYOND_MAX_LIMIT) {
			bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "NG !\n");
		} else {
			bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "pass\n");
		}
	}
	...
	if ((test_types & _MIN_CHECK) != 0) {
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Min Rawdata:  ");
	...
	if ((test_types & _ACCORD_CHECK) != 0) {
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Area Accord:  ");

		if (test_error_code & _BETWEEN_ACCORD_AND_LINE) {
			bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Fuzzy !\n");
		} else {
	...
	if ((test_types & _OFFSET_CHECK) != 0) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Max Offest:  ");
	...
	if ((test_types & _JITTER_CHECK) != 0) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Max Jitier:  ");
	...
	if (test_types & _UNIFORMITY_CHECK) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Uniformity:  ");
	...
	if ((test_types & _KEY_MAX_CHECK) != 0) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Key Max Rawdata:  ");
	...
	if ((test_types & _KEY_MIN_CHECK) != 0) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Key Min Rawdata:  ");
	...
	if (test_types & (_VER_EQU_CHECK | _VER_GREATER_CHECK | _VER_BETWEEN_CHECK)) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Device Version:  ");
	...
	if (test_types & _MODULE_TYPE_CHECK) {
		...
		bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH, "Module Type:  ");
	...
	if ((test_types & _MODULE_SHORT_CHECK) != 0) {
		...
		bytes = (s32) snprintf((char *)data,
				TEST_TEMP_LENGTH, "Module short test:  ");
		...
		if (test_error_code & _GT_SHORT) {
			...
			for (index = 0; index < shortresult[0]; index++) {
				...
				if (shortresult[1 + index * 4] & 0x80) {
					...
					bytes += (s32) snprintf((char *)&data[bytes],
						TEST_TEMP_LENGTH, "Drv%d - ",
						shortresult[1 + index * 4] & 0x7F);
				...
				if (shortresult[2 + index * 4] & 0x80) {
					...
					bytes += (s32) snprintf((char *)&data[bytes],
						TEST_TEMP_LENGTH, "Drv%d 之间短路",
						shortresult[2 + index * 4] & 0x7F);
				...
				bytes += (s32) snprintf((char *)&data[bytes], TEST_TEMP_LENGTH,
							"(R=%d Kohm)\n",
							(((shortresult[3 + index * 4] << 8) +
							shortresult[4 + index * 4]) & 0xffff) / 10);

				DEBUG("%d&%d:", shortresult[1 + index * 4], shortresult[2 + index * 4]);

				DEBUG("%dK", (((shortresult[3 + index * 4] << 8) +
							shortresult[4 + index * 4]) & 0xffff) / 10);
...
static void _unzip_nc(unsigned char *s_buf, unsigned char *key_nc_buf,
					unsigned short s_length, unsigned short key_length)
{
		...
		if (strcmp((const char *)buf, key) == 0) {
			...
			do {
				...
				if (tmpNode < sys.sc_driver_num * sys.sc_sensor_num) {
					...
					tmpNode = tmpNode / sys.sc_driver_num +
	...
	for (i = 0, upload_cnt = 0; i < cnt; i++) {
		...
		for (j = 0, index = 1; j < gt900_short_test_times; j++) {
			...
			for (k = 0; k < s[offest]; k++) {
				if ((s_chn1 == s[offest + k * 4 + 1] && s_chn2 ==
					s[offest + k * 4 + 2]) ||
					(s_chn2 == s[offest + k * 4 + 1] && s_chn1 ==
					s[offest + k * 4 + 2])) {
					tmp[0]++;
...
static s32 _init_test_parameters(char *inipath)
{
...
int gtx_save_string_to_buffer(struct gtx_test_buffer *stp_test, char *tmp_buffer, int length)
{
	...
	if (stp_test->procedure_buffer == NULL || (stp_test->procedure_length + length) > TEST_RESULT_LENGTH) {
...
int gtx_save_test_result(struct gtx_test_buffer *stp_test)
{
	...
	int result = stp_test->test_result;
	     ...
	     if (stp_test->result_buffer == NULL) {
			...
			stp_test->i_txNum = 12;
			stp_test->i_rxNum = 20;
			/*DEBUG("gtx_save_test_result Tx&Tx:%d,Rx&Rx:%d,Tx&Rx:%d",
...
TEST_START:
	...
	test_types = _init_test_parameters(ini_path);
	...
	times = 0;
	timeouts = 0;
	if (test_types & (_MAX_CHECK | _MIN_CHECK | _ACCORD_CHECK | _OFFSET_CHECK |
		_JITTER_CHECK | _KEY_MAX_CHECK | _KEY_MIN_CHECK | _UNIFORMITY_CHECK)) {
