...
static int ts_information_proc_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "chip type : %s\n", "FT5XX6");
	seq_printf(m, "i2c address : %02x\n", 0x3e);
	...
	seq_printf(m, "fw version : %02x\n", buf);
	seq_printf(m, "update flag : 0x%x\n", ftc_update_flag);

	if (ver !=  -1) {
		seq_printf(m, "need update : %s\n", (ver > buf ? "yes" : "no"));
		seq_printf(m, "ready fw version : %02x\n", ver);
	} else {
		seq_printf(m, "no fw to update\n");
	}
