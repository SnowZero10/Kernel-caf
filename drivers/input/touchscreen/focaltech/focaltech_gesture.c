...
static void fts_check_gesture(struct input_dev *input_dev, int gesture_id)
{
	pr_info("fts gesture_id==0x%x\n ", gesture_id);

	switch (gesture_id) {
	case GESTURE_DOUBLECLICK:
		pr_info("fts_check_gesture gesture_id==0x%x\n ", gesture_id);
		input_report_key(input_dev, /*KEY_GESTURE_DOUBLE_TAP*/KEY_WAKEUP, 1);
		input_sync(input_dev);
		input_report_key(input_dev, /*KEY_GESTURE_DOUBLE_TAP*/KEY_WAKEUP, 0);
		input_sync(input_dev);
		break;
	default:
		break;
...
int fts_read_Gestruedata(void)
{
	...
	pr_info("tpd read FTS_GESTRUE buf is 0x%x\n", buf);
	if (ret < 0) {
