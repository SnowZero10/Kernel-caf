...
static unsigned char CTPM_FW_yili[] = {
};
...
static unsigned char CTPM_FW_tongxingda[] = {
};

static unsigned char CTPM_FW_helitai[] = {
	#include "FT3407_Upgrade_helitai_App_V51.i"
};
static unsigned char CTPM_FW_helitai1[] = {
	#include "FT3327_Upgrade_helitai_App_V51.i"
};
static unsigned char CTPM_FW_lianchuang[] = {
	#include "FT3327_Upgrade_lianchuang_App_V51.i"
};
...
struct fts_Upgrade_Info fts_updateinfo[] = {
	...
	{0x0e, FTS_MAX_POINTS_2, AUTO_CLB_NONEED, 100, 30, 0x79, 0x18, 10, 2000}, /*,"FT3207"//zhangjian add*/
...
int FTS_read_chipid_vendorid(struct i2c_client *client, u8 *VendorID, u8 *ChipID)
{
	...
	while (retry++ < 5) {
		...
		if ((*ChipID != 0) && (*VendorID != 0)) {
			pr_info("%s chip_id = %x,vendor_id =%x\n", __func__, *ChipID, *VendorID);
			return 0;
		}
		pr_info("%s chip_id = %x,vendor_id =%x\n", __func__, *ChipID, *VendorID);
		msleep(20);
...
int fts_6x36GU_read_para_from_bootloader(struct i2c_client *client, u8 *pVendorID)
{
	u8 reg_val[4] = {0};
	u32 i = 0;
	u8 auc_i2c_write_buf[10];
...
int fts_5x46_read_para_from_bootloader(struct i2c_client *client, u8 *pVendorID)
{
	u8 reg_val[2] = {0};
	u32 i = 0;
	u8 auc_i2c_write_buf[10];
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if (i_ret < 0)
		FTS_DBG("[FTS] failed writing  0xAA to register 0xfc!\n");
		msleep(20);
		/*write 0x55 to register 0xfc*/
		i_ret = fts_write_reg(client, FTS_RST_CMD_REG1, FTS_UPGRADE_55);
		if (i_ret < 0)
		FTS_DBG("[FTS] failed writing  0x55 to register 0xfc!\n");
		msleep(200);
		i_ret = hidi2c_to_stdi2c(client);
		if (i_ret == 0) {
			FTS_DBG("[FTS] hid change to i2c fail!\n");
		...
		i_ret = fts_i2c_write(client, auc_i2c_write_buf, 2);
		if (i_ret < 0)
			FTS_DBG("[FTS] failed enter into upgrade mode!n");
		/*********Step 3:check READ-ID***********************/
		msleep(20);
		auc_i2c_write_buf[0] = 0x90;
		auc_i2c_write_buf[1] = auc_i2c_write_buf[2] = auc_i2c_write_buf[3] = 0x00;
...
void fts_get_upgrade_array_CTPM_FW(void)
{
	...
	if (ret < 0) {
		pr_info("read chipid vendorid fail");
		if (focaltech_chip_id == 0x64) {
			chip_id = 0x64;
			fts_6x36GU_read_para_from_bootloader(fts_i2c_client, &vendor_id);
			FW_ERR_FORCE_UPDATE = true;
			pr_info("FW maybe upgrade fail ,vendor id read from bootloader,chip id is 0x%x ,vendor id is 0x%x.\n",
			chip_id, vendor_id);
		} else if (focaltech_chip_id == 0x54) {
			chip_id = 0x54;
			fts_5x46_read_para_from_bootloader(fts_i2c_client, &vendor_id);
			FW_ERR_FORCE_UPDATE = true;
			pr_info("FW maybe upgrade fail ,vendor id read from bootloader,chip id is 0x%x ,vendor id is 0x%x.\n",
			chip_id, vendor_id);
		}
	...
	case 0x11:
	case 0x54:
		pr_info("[FTS] vendor id is yili.\n");
		CTPM_FW = CTPM_FW_yili;
		ctpfw_filenth = sizeof(CTPM_FW_yili);
		break;
	case 0x15:
		pr_info("[FTS] vendor  is chuangwei.\n");
		CTPM_FW = CTPM_FW_chuangwei;
		ctpfw_filenth = sizeof(CTPM_FW_chuangwei);
		break;
	case 0x67:
		pr_info("[FTS] vendor  is dijing.\n");
		CTPM_FW = CTPM_FW_dijing;
		ctpfw_filenth = sizeof(CTPM_FW_dijing);
		break;
	case 0x85:
		pr_info("[FTS] vendor id is junda.\n");
		CTPM_FW = CTPM_FW_junda;
		ctpfw_filenth = sizeof(CTPM_FW_junda);
		break;
	case 0x90:
		pr_info("[FTS] vendor id is tongxingda.\n");
		CTPM_FW = CTPM_FW_tongxingda;
		ctpfw_filenth = sizeof(CTPM_FW_tongxingda);
		break;
	case 0xB5:
		pr_info("[FTS] vendor  is yashi.\n");
		CTPM_FW = CTPM_FW_yashi;
		ctpfw_filenth = sizeof(CTPM_FW_yashi);
		break;
	case 0xe0:
	case 0x82:
		pr_info("[FTS] vendor  is helitai.\n");
		if (chip_id == 0x0e) {
			CTPM_FW = CTPM_FW_helitai;
			ctpfw_filenth = sizeof(CTPM_FW_helitai);
		} else if (chip_id == 0x54) {
			CTPM_FW = CTPM_FW_helitai1;
			ctpfw_filenth = sizeof(CTPM_FW_helitai1);
		}
		break;
	case 0x87:
		pr_info("[FTS] vendor  is lianchuang.\n");
		CTPM_FW = CTPM_FW_lianchuang;
		ctpfw_filenth = sizeof(CTPM_FW_lianchuang);
		break;
	default:
		pr_info("[FTS] vendor id is error.\n");
		break;
	}
}
...
static int fts_GetFirmwareSize(char *firmware_name)
{
	...
	memset(filepath, 0, sizeof(filepath));
       /*snprintf(filepath, "%s%s", FTXXXX_INI_FILEPATH_CONFIG, firmware_name);*/
	snprintf(filepath, 256, "/sdcard/%s", firmware_name);
	 pr_info("filepath is %s\n", filepath);
	if (NULL == pfile) {
...
static int fts_ReadFirmware(char *firmware_name, unsigned char *firmware_buf)
{
	...
	memset(filepath, 0, sizeof(filepath));
	snprintf(filepath, 256, "%s%s", FTXXXX_INI_FILEPATH_CONFIG, firmware_name);
	if (NULL == pfile) {
		pfile = filp_open(filepath, O_RDONLY, 0);
	}
	if (IS_ERR(pfile)) {
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
	...
	|| (fts_updateinfo_curr.CHIP_ID == 0x13) || (fts_updateinfo_curr.CHIP_ID == 0x14)) {
	...
	} else if ((fts_updateinfo_curr.CHIP_ID == 0x36) || (fts_updateinfo_curr.CHIP_ID == 0xe)) {
		...
		i_ret = fts_8606_writepram(client, pbt_buf, fwsize);
		if (i_ret != 0) {
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
	pr_info("fts_updateinfo_curr.CHIP_ID is 0x%x\n", fts_updateinfo_curr.CHIP_ID);
	if ((fts_updateinfo_curr.CHIP_ID == 0x11) || (fts_updateinfo_curr.CHIP_ID == 0x12)
	|| (fts_updateinfo_curr.CHIP_ID == 0x13) || (fts_updateinfo_curr.CHIP_ID == 0x14)
	...
	|| (fts_updateinfo_curr.CHIP_ID == 0x0a) || (fts_updateinfo_curr.CHIP_ID == 0x08)) {
		...
		if ((CTPM_FW[fw_len - 8] ^ CTPM_FW[fw_len - 6]) == 0xFF
			...
			if ((fts_updateinfo_curr.CHIP_ID == 0x55) || (fts_updateinfo_curr.CHIP_ID == 0x08)
			|| (fts_updateinfo_curr.CHIP_ID == 0x0a)) {
				i_ret = fts_5x06_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);
			} else if ((fts_updateinfo_curr.CHIP_ID == 0x11) || (fts_updateinfo_curr.CHIP_ID == 0x12)
			|| (fts_updateinfo_curr.CHIP_ID == 0x13) || (fts_updateinfo_curr.CHIP_ID == 0x14)) {
				i_ret = fts_5x36_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);
			} else if ((fts_updateinfo_curr.CHIP_ID == 0x06)) {
				i_ret = fts_6x06_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);
			}
			if (i_ret != 0)
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_6x36_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n", __func__);
	} else if ((fts_updateinfo_curr.CHIP_ID == 0x64)) {
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_6336GU_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);
		if (i_ret != 0)
				dev_err(&client->dev, "%s:upgrade failed. err.\n", __func__);
	} else if ((fts_updateinfo_curr.CHIP_ID == 0x54)) {
		...
		pbt_buf = CTPM_FW;
		/*call the upgrade function*/
		i_ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);
		if (i_ret != 0) {
		...
		pbt_buf = CTPM_FW;
		/*call the upgrade function*/
		i_ret = fts_5822_ctpm_fw_upgrade(client, pbt_buf, ctpfw_filenth);
		if (i_ret != 0) {
			dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
		} else {
			#ifdef AUTO_CLB
		...
		pbt_buf = CTPM_FW;
		/*call the upgrade function*/
		i_ret = fts_8606_writepram(client, CTPM_FW, sizeof(CTPM_FW));
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	pr_info("[FTS] uc_tp_fm_ver is 0x%x.\n", uc_tp_fm_ver);
