	...
	case PROC_UPGRADE:
		{
			...
			snprintf(upgrade_file_path, 256, "%s", writebuf + 1);
...
static ssize_t fts_debug_read(struct file *filp, char __user *buff, size_t count, loff_t *ppos)
{
	...
	case PROC_UPGRADE:
		...
		if (ret < 0)
			num_read_chars = snprintf(buf, 256, "%s", "get fw version failed.\n");
		else
			num_read_chars = snprintf(buf, 256, "current fw version:0x%02x\n", regvalue);
		break;
	case PROC_READ_REGISTER:
		readlen = 1;
...
* Return: EPERM
***********************************************************************/
static ssize_t fts_tpdriver_version_store(struct device *dev, struct device_attribute *attr,
...
static ssize_t fts_tprwreg_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (2 == num_read_chars) {
		...
		if (fts_read_reg(client, regaddr, &regvalue) < 0)
			pr_info("[Focal] %s : Could not read the register(0x%02x)\n", __func__, regaddr);
		else
...
static ssize_t fts_fwupgradeapp_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	snprintf(fwname, 256, "%s", buf);
	fwname[count-1] = '\0';

	mutex_lock(&fts_input_dev->mutex);
	if (NULL == client) {
		pr_info("[FTS]wkh, enter fts_fwupgradeapp_store\n");
		return 0;
	}
	disable_irq(client->irq);
	#if GTP_ESD_PROTECT
