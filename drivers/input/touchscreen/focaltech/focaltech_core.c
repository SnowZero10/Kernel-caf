...
/*ZSW_MODIFY begin,modify for lcd resume long time,yangchaofeng,20160608*/
#ifdef CONFIG_FB
static int tpd_suspend_flag = 1;
static struct device *touch_resume_device;
...
#endif
/*ZSW_MODIFY end,modify for lcd resume long time,yangchaofeng,20160608*/
/*******************************************************************************
...
u8 focaltech_chip_id = 0;
u8 buf_touch_data[30*POINT_READ_BUF] = { 0 };
u8 focaltech_device_id = 0;
...
static void touch_resume_workqueue_callback(struct work_struct *work)
{
	fts_ts_resume(touch_resume_device);
	tpd_suspend_flag = 0;
}
/*ZSW_MODIFY end,modify for lcd resume long time,yangchaofeng,20160608*/
/*******************************************************************************
...
{
	int err = 0; /*yangchaofeng add for lcd resume long time,yangchaofeng,20160608*/
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
		...
		blank = evdata->data;
		/*pr_info("[MSM_TP]%s: tpd_suspend_flag = %d event = %ld *blank = %d\n",
		__func__, tpd_suspend_flag, event, *blank);*/
		if (*blank == FB_BLANK_UNBLANK && tpd_suspend_flag) {
				/*ZSW_MODIFY begin,modify for lcd resume long time,yangchaofeng,20160608*/
				#if  0
				fts_ts_resume(&fts_data->client->dev);
				#else
				touch_resume_device = &fts_data->client->dev;
				err = queue_work(touch_resume_workqueue, &touch_resume_work);
				if (!err)
					pr_info("start touch_resume_workqueue failed\n");
				#endif
				/*ZSW_MODIFY end,modify for lcd resume long time,yangchaofeng,20160608*/
			...
			if (!err)
				/*pr_info("cancel touch_resume_workqueue err = %d\n", err);*/
			/*ZSW_MODIFY end,modify for lcd resume long time,yangchaofeng,20160608*/
			fts_ts_suspend(&fts_data->client->dev);
			tpd_suspend_flag = 1;/*yangchaofeng add for lcd resume long time,yangchaofeng,20160608*/
		}
	}
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
	...
	rc = of_property_read_u32(np, "focaltech,chip-id", &temp_val);
	if (!rc)
		focaltech_chip_id = temp_val;
	else
		return rc;
...
static ssize_t fts_tpd_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	/*pr_info("vendor_id is %d\n", vendor_id);*/
	switch (vendor_id) {
	case 0x15:
		len += snprintf(buffer_tpd+len, 64, "TP module: chuangwei (0x15)\n");
		break;
	case 0x54:
		len += snprintf(buffer_tpd+len, 64, "TP module: yili (0x54)\n");
		break;
	case 0x67:
		len += snprintf(buffer_tpd+len, 64, "TP module: dijing (0x67)\n");
		break;
	case 0x80:
		len += snprintf(buffer_tpd+len, 64, "TP module: yeji(0x80)\n");
		break;
	case 0x85:
		len += snprintf(buffer_tpd+len, 64, "TP module: dunda(0x85\n");
		break;
	case 0x90:
		len += snprintf(buffer_tpd+len, 64, "TP module: tongxingda(0x90\n");
		break;
	case 0xB5:
		len += snprintf(buffer_tpd+len, 64, "TP module: yashi (0xb5)\n");
		break;
	default:
		len += snprintf(buffer_tpd+len, 64, "vendor id is 0x%x\n", vendor_id);
		break;
	}
	if (focaltech_device_id == 0x54)
		len += snprintf(buffer_tpd+len, 64, "IC type : %s(FT3327)\n", "focaltech");
	else if (focaltech_device_id == 0xe)
		len += snprintf(buffer_tpd+len, 64, "IC type : %s(FT3407)\n", "focaltech");
	else
		len += snprintf(buffer_tpd+len, 64, "IC type : %s\n", "focaltech");

	len += snprintf(buffer_tpd+len, 64, "I2C address: 0x%x\n", 0x38);
	len += snprintf(buffer_tpd+len, 64, "Firmware version : 0x%x\n", fts_fm_ver);
...
#define FTS_PROC_GESTURE_FILE "wake_gesture"
struct proc_dir_entry *fts_proc_gesture_file = NULL;
static ssize_t fts_gesture_read(struct file *file, char *buf,
	size_t size, loff_t *pos)
{
	int len = 0;
	char temp_buf[2];

	len += snprintf(temp_buf+len, 64, "%d\n", fts_wq_data->wake_gesture_enable);
	return simple_read_from_buffer(buf, size, pos, temp_buf, len);
}

static ssize_t fts_gesture_write(struct file *file, const char *buff,
	size_t len, loff_t *pos)
{
	...
	if (buf[0] == '0') {
		fts_wq_data->wake_gesture_enable = 0;
	} else if (buf[0] == '1') {
		fts_wq_data->wake_gesture_enable = 1;
	} else
		return -EINVAL;
	/*pr_info("%s: SMART_WAKEUP_enable = %d.\n", __func__, fts_wq_data->wake_gesture_enable);*/

	return len;
}

static struct file_operations fts_proc_gesture_ops = {
	.owner = THIS_MODULE,
...
static int tpd_init_tpinfo(struct tpd_classdev_t *cdev)
{
	...
	while (retry++ < 5) {
		fts_read_reg(fts_i2c_client, FTS_REG_ID, &chipid_in_chip);
		/*fts_read_reg(data->client, FTS_REG_PSENSOR_ENABLE, &state);*/
		fts_read_reg(fts_i2c_client, FTS_REG_FW_VENDOR_ID, &vendorid_in_chip);
		fts_read_reg(fts_i2c_client, FTS_REG_FW_VER, &fwver_in_chip);
		if ((chipid_in_chip != 0) && (vendorid_in_chip != 0) && (fwver_in_chip != 0)) {
			/*pr_info("%s chip_id = %x, vendor_id = %x,fw_version = %x .\n", __func__,
...
static int tpd_get_wakegesture(struct tpd_classdev_t *cdev)
{
	...
	/*pr_info("%s wakeup_gesture_enable val is:%d.\n", __func__, data->wake_gesture_enable);*/
	cdev->b_gesture_enable = data->wake_gesture_enable;
	return 0;
}
static int tpd_enable_wakegesture(struct tpd_classdev_t *cdev, int enable)
{
	...
	/*pr_info("%s previous val is:%d, current val is:%d.\n", __func__, data->wake_gesture_enable, enable);*/
	data->wake_gesture_enable = enable;
	return enable;
}
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	u8 reg_addr;
	int err, len;
	struct  proc_dir_entry *TouchscreenInfo = NULL;
	...
	data->wake_gesture_enable = false;
	data->wake_gesture_enter = false;
	...
	#endif
#if 0
	ftsc_wake_gesture_kobj = kobject_create_and_add("tp_wake_gesture", NULL);
	if (!ftsc_wake_gesture_kobj) {
		/*pr_info("Unable to create shtsc_wake_gesture_kobj\n");*/
	}
	err = sysfs_create_group(ftsc_wake_gesture_kobj, &attr_group);
	if (err) {
		kobject_put(ftsc_wake_gesture_kobj);
		/*pr_info( "Unable to create sysfs group\n");*/
	}
#endif
	TouchscreenInfo = proc_mkdir("touchscreen", NULL);
	if (NULL == proc_create("touchscreen/ts_information", 0644, NULL, &fts_tpd_id)) {
		/*pr_info("Failed to create procfs file node!\n");*/
	}

	fts_proc_gesture_file = proc_create(FTS_PROC_GESTURE_FILE, (S_IWUSR|S_IRUGO|S_IWUGO),
		TouchscreenInfo, &fts_proc_gesture_ops);
	if (fts_proc_gesture_file == NULL) {
		/*pr_info(" %s: proc gesture file create failed!\n", __func__);*/
	}

	mt_entry = proc_create("driver/tsc_id", 0644, NULL, &fts_tpd_id);/*zhangjian add for DFX*/
	if (mt_entry) {
		/*pr_info("create tsc_id success\n");*/
		dev_err(&client->dev, "create tsc_id success\n");
	} else
		/*pr_info("create tsc_id fail\n");*/
		dev_err(&client->dev, "create tsc_id fail\n");
