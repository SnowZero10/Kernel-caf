...
static const struct tp_vendor_fw_t synaptics_vendor_fw_list[] = {
	...
	/*The problematic modules in the Z5000A and Z559DL projects will use 0x59 as the vendor ID*/
	{0x59, "HOLITCH", SYN_HOLITCH_FW_NAME},
	{0xff, "Unknown", "NULL"},
};
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (touch_count == 0) {
		...
		input_mt_sync(rmi4_data->input_dev);
#endif
		pr_info("%s: touch up, finger_nums %d[%u:%u]\n", __func__, finger_nums, log_x, log_y);
		log_x = 0;
		log_y = 0;
		finger_nums = 0;
 ...
 */
		...
		y = 920;
#elif defined(CONFIG_BOARD_SWEET) || defined(CONFIG_BOARD_LENA) || defined(CONFIG_BOARD_CASTELLA)
		switch (button) {
		case 0:
			x = 85;
			break;
		...
		bool enable, bool attn_only)
{
	...
	if (enable) {
		...
		pr_info("%s: irq_flags 0x%08lX\n", __func__, bdata->irq_flags);
...

{
	...
	if (ready_fw_ver == -1) {
		...
		len += snprintf(buffer + len, sizeof(buffer) - len, "ready fw version : 0x%04X\n", ready_fw_ver);
	}
