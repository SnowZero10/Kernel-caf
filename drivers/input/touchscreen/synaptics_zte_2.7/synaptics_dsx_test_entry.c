...
void init_tddi_detail_threshold_rawdata(struct synaptics_test_st *stp_test)
{
	...
	for (i = 0; i < stp_test->screen_cfg.iUsedMaxTxNum; i++) {
		snprintf(str, TEST_TEMP_LENGTH, "RawData_Max_Tx%d", (i + 1));
		dividerPos = get_key_value_string("SpecialSet", str, "111", strTemp);
		snprintf(strValue, TEST_TEMP_LENGTH, "%s", strTemp);
		if (dividerPos == 0)
			continue;
		index = 0;
		k = 0;
	...
	for (i = 0; i < stp_test->screen_cfg.iUsedMaxTxNum; i++) {
		snprintf(str, TEST_TEMP_LENGTH, "RawData_Min_Tx%d", (i + 1));
...
void init_tddi_invalid_node(struct synaptics_test_st *stp_test)
{
	...
	for (i = 0; i < stp_test->screen_cfg.iUsedMaxTxNum; i++) {
		for (j = 0; j < stp_test->screen_cfg.iUsedMaxRxNum; j++) {
			snprintf(strTemp, TEST_TEMP_LENGTH, "invalid_node[%d][%d]", (i + 1), (j + 1));
...
static int _tpd_test_save_to_file(struct synaptics_test_st *stp_test)
{
	...
	snprintf(filepath, TEST_TEMP_LENGTH, "%s%s", stp_test->str_saved_path, saved_name);
...
int init_tpd_test(struct tpd_classdev_t *cdev)
{
	...
	strlcpy(stp_test->str_saved_path, "/data/data/", 256);
	strlcpy(stp_test->str_ini_file_name, "syna_test_sensor_0.ini", 128);
	strlcpy(stp_test->str_ini_file_path, "/system/etc/", 256);
