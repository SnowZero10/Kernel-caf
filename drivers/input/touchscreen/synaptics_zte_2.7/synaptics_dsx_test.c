...
int synaptics_full_rawdata_test(struct synaptics_test_st *stp_test, int times)
{
	...
	for (i_row = 0; i_row < stp_test->screen_cfg.i_txNum; i_row++) {
		for (i_col = 0; i_col < stp_test->screen_cfg.i_rxNum; i_col++) {
			...
			if ((stp_test->test_item.touch_key_test) && (i_col == (stp_test->screen_cfg.i_rxNum - 1))) {
	...
	if ((stp_test->test_item.touch_key_test) && (button_test < 3)) {
		b_result = false;
		stp_test->test_result |= TEST_BEYOND_ACCORD_LIMIT;
		TPD_TEST_DBG("button rawdata test failure.");
	}
