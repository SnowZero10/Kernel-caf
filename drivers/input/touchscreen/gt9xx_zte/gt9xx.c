...
static struct workqueue_struct *gt9xx_workqueue = NULL;
...
static s8 gtp_enter_sleep(struct goodix_ts_data *ts)
{
	...
	/* set reset pin as suspend mode */
	if (gtp_ts->ts_pinctrl) {
		gt_pinctrl_select(gtp_ts, false);
	}
	GTP_GPIO_OUTPUT(gtp_ts->irq_gpio, 0);
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data *ts)
{
	...
	/* set reset pin as active mode */
	if (gtp_ts->ts_pinctrl) {
		gt_pinctrl_select(gtp_ts, true);
	}
...
static int tpd_init_tpinfo(struct tpd_classdev_t *cdev)
{
	...
	cdev->ic_tpinfo.chip_part_id = CHIP_TYPE_GT9X;
	GTP_INFO("sensor_id = %d", gtp_ts->sensor_id);
	switch (gtp_ts->sensor_id) {
	case 0:
		/* GUOXIAN */
		cdev->ic_tpinfo.module_id = GT9XX_VENDOR_ID_0;
		break;
	default:
		/* unknown */
		cdev->ic_tpinfo.module_id = GT9XX_VENDOR_ID_UNKNOWN;
		break;
	}
	cdev->ic_tpinfo.chip_ver = 0;
	cdev->ic_tpinfo.firmware_ver = gtp_ts->version;
	cdev->ic_tpinfo.i2c_type = 0;
	cdev->ic_tpinfo.i2c_addr = gtp_ts->client->addr;
	return 0;
}
...
#define GT9X_DEBUG_PROC_FILE		"gt9x_debug"
#define TP_INFO_LENGTH 4096
static struct proc_dir_entry *gt9x_debug_proc_entry = NULL;

static ssize_t gt9x_debug_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	char *ptr;
	char temp_data[GTP_CONFIG_MAX_LENGTH] = { 0 };
	char temp_data_read[GTP_CONFIG_MAX_LENGTH + 2] = { 0 };
	int i;
	int ret = 0;
	int len = 0;
	...
	len += snprintf(ptr + len, TP_INFO_LENGTH - len,
		" == == GT9X config read from chip == ==\n");

	temp_data_read[0] = (u8)(GTP_REG_CONFIG_DATA >> 8);
	temp_data_read[1] = (u8)(GTP_REG_CONFIG_DATA & 0xFF);
	ret = gtp_i2c_read(i2c_connect_client, temp_data_read, GTP_CONFIG_MAX_LENGTH+2);
	memcpy(temp_data, temp_data_read+2, GTP_CONFIG_MAX_LENGTH);

	GTP_INFO("I2C TRANSFER: %d", ret);
	...
	len += snprintf(ptr + len, TP_INFO_LENGTH - len, "\n");
	/* Touch PID & VID */
	len += snprintf(ptr + len, TP_INFO_LENGTH - len, " == == GT9X Version Info == ==\n");

	memset(temp_data_read, 0, GTP_CONFIG_MAX_LENGTH + 2);
	temp_data_read[0] = (u8)(GTP_REG_VERSION >> 8);
	temp_data_read[1] = (u8)(GTP_REG_VERSION & 0xFF);
	ret = gtp_i2c_read(i2c_connect_client, temp_data_read, 14);
	if (ret < 0) {
		kfree(ptr);
		return -EINVAL;
	}
	memcpy(temp_data, temp_data_read+2, 12);
...
static const struct file_operations gt9x_debug_fops = {
	.owner = THIS_MODULE,
	.read = gt9x_debug_read_proc,
};

static s32 gt9x_init_debug_node(void)
{
	gt9x_debug_proc_entry = proc_create(GT9X_DEBUG_PROC_FILE, 0440, NULL, &gt9x_debug_fops);
	if (gt9x_debug_proc_entry == NULL) {
		GTP_ERROR("Create proc entry /proc/%s FAILED!", GT9X_DEBUG_PROC_FILE);
		return -EINVAL;
	}
	GTP_INFO("Created proc entry /proc/%s.", GT9X_DEBUG_PROC_FILE);
	return 0;
}
...
static int gt_pinctrl_init(struct goodix_ts_data *rmi4_data)
{
	...
	rmi4_data->gpio_state_init
		= pinctrl_lookup_state(rmi4_data->ts_pinctrl, "pmx_ts_init");
	if (IS_ERR_OR_NULL(rmi4_data->gpio_state_init)) {
		dev_dbg(&rmi4_data->client->dev,
			"Can not get ts init pinstate\n");
		retval = PTR_ERR(rmi4_data->gpio_state_init);
		rmi4_data->ts_pinctrl = NULL;
		return retval;
	}

	rmi4_data->gpio_state_release
		= pinctrl_lookup_state(rmi4_data->ts_pinctrl, "pmx_ts_release");
	if (IS_ERR_OR_NULL(rmi4_data->gpio_state_release)) {
		dev_dbg(&rmi4_data->client->dev,
			"Can not get ts release pinstate\n");
		retval = PTR_ERR(rmi4_data->gpio_state_release);
		rmi4_data->ts_pinctrl = NULL;
		return retval;
	}
...
#if GTP_CHARGER_SWITCH
static u8 gt9xx_config_charger[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
	= {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
static struct delayed_work charger_switch_work;
static int charger_work_cycle = 2 * HZ;
static spinlock_t charger_lock;
static int charger_running = 0;

u32 gt9xx_get_charger_status(void)
{
	...
	if (batt_psy) {
		...
		GTP_INFO("battery status = %d\n", ret.intval);
		return ret.intval;
	}

	/* Default to false if the battery power supply is not registered. */
	GTP_INFO("battery power supply is not registered\n");
	return POWER_SUPPLY_STATUS_UNKNOWN;
}

static int gt9xx_parse_chr_cfg(int sensor_id)
{
#if GTP_DRIVER_SEND_CFG
	u8 check_sum = 0;
	int len;
	...
	u8 cfg_info_group1[] = CTP_CFG_GROUP1_CHG;
	u8 cfg_info_group2[] = CTP_CFG_GROUP2_CHG;
	u8 cfg_info_group3[] = CTP_CFG_GROUP3_CHG;
	u8 cfg_info_group4[] = CTP_CFG_GROUP4_CHG;
	u8 cfg_info_group5[] = CTP_CFG_GROUP5_CHG;
	u8 cfg_info_group6[] = CTP_CFG_GROUP6_CHG;
	u8 *send_cfg_buf[] = {cfg_info_group1, cfg_info_group2, cfg_info_group3,
						cfg_info_group4, cfg_info_group5, cfg_info_group6};
	u8 cfg_info_len[] = { CFG_GROUP_LEN(cfg_info_group1),
						CFG_GROUP_LEN(cfg_info_group2),
	...
	GTP_DEBUG_FUNC();
	if (sensor_id >= ARRAY_SIZE(send_cfg_buf))
		return -EINVAL;

	len = cfg_info_len[sensor_id];
	if (len == 0 || len < GTP_CONFIG_MIN_LENGTH) {
		GTP_ERROR("Length of config is incorrect.");
		return -EINVAL;
	}

	memset(&gt9xx_config_charger[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
	memcpy(&gt9xx_config_charger[GTP_ADDR_LENGTH], send_cfg_buf[sensor_id], len);

	#if GTP_CUSTOM_CFG
	gt9xx_config_charger[RESOLUTION_LOC]	   = (u8)GTP_MAX_WIDTH;
	gt9xx_config_charger[RESOLUTION_LOC + 1] = (u8)(GTP_MAX_WIDTH>>8);
	gt9xx_config_charger[RESOLUTION_LOC + 2] = (u8)GTP_MAX_HEIGHT;
	gt9xx_config_charger[RESOLUTION_LOC + 3] = (u8)(GTP_MAX_HEIGHT>>8);

	if (GTP_INT_TRIGGER == 0)
		gt9xx_config_charger[TRIGGER_LOC] &= 0xfe;
	else if (GTP_INT_TRIGGER == 1)
		gt9xx_config_charger[TRIGGER_LOC] |= 0x01;
	#endif

	check_sum = 0;
	for (i = GTP_ADDR_LENGTH; i < len; i++) {
		check_sum += gt9xx_config_charger[i];
	}
	gt9xx_config_charger[len] = (~check_sum) + 1;
#endif

	return 0;
}

static s32 gtp_send_charger_cfg(struct i2c_client *client)
{
	s32 ret = 2;
	...
	for (retry = 0; retry < 5; retry++) {
		ret = gtp_i2c_write(client, gt9xx_config_charger,
...
/**
 * gt9xx_charger_config - check and update charging status configuration
 * @dir_update
 *	  0: check before send charging status configuration
 *	  1: directly send charging status configuration
 *
 */
int gt9xx_charger_config(s32 dir_update)
{
	static u8 chr_pluggedin = 0;
	int ret = 0;

	if (gt9xx_get_charger_status() == POWER_SUPPLY_STATUS_CHARGING) {
		if (!chr_pluggedin || dir_update) {
			GTP_INFO("Charger Plugin.");

			ret = gtp_send_charger_cfg(i2c_connect_client);
			chr_pluggedin = 1;
		}
	} else {
		if (chr_pluggedin || dir_update) {
...
static void gt9xx_charger_work_func(struct work_struct *work)
{
	if (!charger_running) {
	...
	gt9xx_charger_config(0);

	GTP_DEBUG("Charger check done!");
	if (charger_running)
		queue_delayed_work(gt9xx_workqueue, &charger_switch_work, charger_work_cycle);
}

static void gt9xx_init_charger(void)
{
	GTP_INFO("Clock ticks for an charger cycle: %d", charger_work_cycle);
	INIT_DELAYED_WORK(&charger_switch_work, gt9xx_charger_work_func);
	spin_lock_init(&charger_lock);

	if (gt9xx_parse_chr_cfg(gtp_ts->sensor_id) < 0)
		GTP_ERROR("Error occurred when parse charger config.");
}

/**
 * gt9xx_charger_switch - switch states of charging work thread
 *
 * @on: SWITCH_ON - start work thread, SWITCH_OFF: stop .
 *
 */
void gt9xx_charger_switch(s32 on)
{
	...
	if (on == SWITCH_ON) {
		if (!charger_running) {
			...
			GTP_INFO("Charger check started!");
			queue_delayed_work(gt9xx_workqueue, &charger_switch_work, charger_work_cycle);
		} else
			spin_unlock(&charger_lock);
	} else {
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!ret && ts->ts_pinctrl) {
		if (!IS_ERR_OR_NULL(ts->gpio_state_init)) {
			ret = pinctrl_select_state(ts->ts_pinctrl, ts->gpio_state_init);
			if (ret) {
				dev_err(&ts->client->dev,	"can not set %s pins\n",
					"pmx_ts_init");
				goto pinctrl_fail;
			}
		} else
	...
	refresh = proc_create("touchscreen/ts_update", 0444, NULL, &update_proc_ops);
	if (refresh == NULL)
		GTP_INFO("proc_create ts_information failed!\n");
#endif
	gt9x_init_debug_node();

#if GTP_CHARGER_SWITCH
	gt9xx_init_charger();
	gt9xx_charger_config(1);
	gt9xx_charger_switch(SWITCH_ON);
#endif
	GTP_INFO("zte_tzb goodix probe end\n");

	return 0;
i2c_test_fail:
	if (ts->ts_pinctrl && !IS_ERR_OR_NULL(ts->gpio_state_release))	{
		pinctrl_select_state(ts->ts_pinctrl, ts->gpio_state_release);
	}
pinctrl_fail:
...
static int goodix_ts_remove(struct i2c_client *client)
{
	...
	destroy_workqueue(gt9xx_workqueue);
...
#else
	#if GTP_CHARGER_SWITCH
	gt9xx_charger_switch(SWITCH_OFF);
	#endif
...
void gtp_esd_switch(struct i2c_client *client, s32 on)
{
	...
	if (on == SWITCH_ON) {
		if (!ts->esd_running) {
			...
			queue_delayed_work(gt9xx_workqueue, &gtp_esd_check_work, ts->clk_tick_cnt);
		} else
			spin_unlock(&ts->esd_lock);
	} else {
		if (ts->esd_running) {
...
static int	goodix_ts_init(void)
{
	...
	gt9xx_workqueue = create_singlethread_workqueue("gt9xx_workthread");
	if (gt9xx_workqueue == NULL) {
		GTP_ERROR("Create gt9xx_workthread failed!");
	}
#if GTP_ESD_PROTECT
