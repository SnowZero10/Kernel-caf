...
static int fts_power_source_ctrl(struct fts_ts_data *data, int enable)
{
	...
	if (enable) {
		if (!data->fts_power_on) {
			...
			rc = regulator_enable(data->ldo_avdd);
			if (rc) {
				FTS_ERROR("Regulator ldo_avdd enable failed rc=%d", rc);
			}

			rc = regulator_enable(data->ldo_iovdd);
			if (rc) {
				FTS_ERROR("Regulator ldo_iovdd enable failed rc=%d", rc);
			}
			data->fts_power_on = true;
		}
	} else {
		if (data->fts_power_on) {
			...
			rc = regulator_disable(data->ldo_avdd);
			if (rc) {
				FTS_ERROR("Regulator ldo_avdd disable failed rc=%d", rc);
			}
			rc = regulator_disable(data->ldo_iovdd);
			if (rc) {
				FTS_ERROR("Regulator ldo_iovdd disable failed rc=%d", rc);
			}
			data->fts_power_on = false;
		}
	}
	FTS_FUNC_EXIT();
	return 0;
