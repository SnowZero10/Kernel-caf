 ...
 *
 * @note
 *   While changing the parameter of the bmi160_t consider the following point:
...
/*!
 * @brief This API used to reads the fatal error from the Register 0x02 bit 0
...
/*!
 * @brief This API used to read the error code from register 0x02 bit 1 to 4
 *
...
/*!
 * @brief This API Reads the i2c error code from the Register 0x02 bit 5.
...
/*!
 * @brief This API reads the magnetometer data ready interrupt not active.
...
if (p_bmi160 == BMI160_NULL) {
	...
	switch (v_channel_u8) {
	/* write input enable of interrupt1 and interrupt2*/
	case BMI160_INTR1_INPUT_ENABLE:
		com_rslt = p_bmi160->BMI160_BUS_READ_FUNC(p_bmi160->
		dev_addr, BMI160_USER_INTR1_INPUT_ENABLE__REG,
		&v_data_u8, BMI160_GEN_READ_WRITE_DATA_LENGTH);
...
u8 v_mag_sec_if_pow_mode_u8)
{
	...
	com_rslt = bmi160_set_command_register(ACCEL_MODE_NORMAL);
	p_bmi160->delay_msec(BMI160_GEN_READ_WRITE_DELAY);
	pr_info("com_rslt:%d, manual:%d, <%s><%d>\n",
...
u8 v_mag_pow_mode_u8)
{
	...
	if (p_bmi160->mag_manual_enable != BMI160_MANUAL_ENABLE) {
		...
		p_bmi160->delay_msec(BMI160_GEN_READ_WRITE_DELAY);
		pr_info("1com_rslt:%d, manual:%d, manual_read:%d\n",
		com_rslt, p_bmi160->mag_manual_enable, manual_enable_status);
	}
	pr_info("2com_rslt:%d, manual:%d, manual_read:%d\n",
	com_rslt, p_bmi160->mag_manual_enable, manual_enable_status);
	...
	break;
	case SUSPEND_MODE:
		pr_info("3com_rslt:%d, manual:%d, read_manual:%d\n",
		com_rslt, p_bmi160->mag_manual_enable, manual_enable_status);
	...
	pr_info("4com_rslt:%d, manual:%d, manual_read:%d\n",
	com_rslt, p_bmi160->mag_manual_enable, manual_enable_status);
	/* set mag interface auto mode*/
	if (p_bmi160->mag_manual_enable == BMI160_MANUAL_ENABLE) {
		com_rslt += bmi160_set_mag_manual_enable(
	...
	pr_info("5com_rslt:%d, manual:%d, manual_read:%d\n",
	com_rslt, p_bmi160->mag_manual_enable, manual_enable_status);
	return com_rslt;
}
/*!
...
u8 v_akm_i2c_address_u8)
{
	...
	pr_info("bmi160,addr:0x%x, akm_chip_id:0x%x",
...
struct bmi160_mag_xyz_s32_t *bst_akm_xyz)
{
	...
	com_rslt = bmi160_read_mag_xyz(&mag_xyz, BST_AKM);
	pr_info("akm09912_raw_x:%d, %d, %d, <%s>,<%d>",
...
u8 v_mag_sec_if_pow_mode_u8)
{
	...
	pr_err("com_rslt:%d, manual:%d,after setacc normal mode\n",
...
u8 v_acquisition_command_u8)
{
	...
	*v_overflow_s8 = BMI160_INIT_VALUE;
	...
	/* Calculate temperature correction */
		...
		v_temp_u16;
	if (yas532_data.temp_data.idx >= YAS532_MAG_TEMPERATURE_LOG)
		yas532_data.temp_data.idx = 0;
		yas532_data.temp_data.num++;
	if (yas532_data.temp_data.num >= YAS532_MAG_TEMPERATURE_LOG)
...
u16 *v_temperature_u16, u16 *xy1y2, u8 *v_ouflow_u8)
{
	...
	*v_ouflow_u8 = 0;
	for (i = 0; i < 3; i++) {
		if (xy1y2[i] >= YAS537_DATA_OVERFLOW)
			*v_ouflow_u8 |= (1 << (i * 2));
...
u8 *v_ouflow_u8, struct yas_vector *vector_xyz)
{
	...
	*v_ouflow_u8 = 0;
	...
	case YAS537_MAG_STATE_NORMAL:
		if (v_ouflow_u8 > BMI160_INIT_VALUE
