...
#define BMI160_STEPCOUNTER_INPUT_NAME	"bmi160-stepcounter"
#define ABSMIN                      -512
#define ABSMAX                      512
...
struct bmi_client_data {
	...
	struct input_dev *input_stepcounter;
	...
	struct hrtimer stepcounter_timer;
	ktime_t accel_ktime;
	ktime_t gyro_ktime;
	ktime_t stepcounter_ktime;
	struct work_struct accel_work;
	struct work_struct gyro_work;
	struct work_struct stepcounter_work;
	unsigned int accel_delay;
	unsigned int gyro_delay;
	unsigned int stepcounter_delay;
	...
	u32 acc_cal_y;
	u32 acc_cal_z;
	int acc_cal_result; /* 0:OK, -1 failed */
	char acc_calibrate_buf[20];
	u32 gyro_cal_x;
	u32 gyro_cal_y;
	u32 gyro_cal_z;
	int gyro_cal_result; /* 0:OK, -1 failed */
	char gyro_calibrate_buf[20];
};
