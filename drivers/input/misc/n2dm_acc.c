...
#define CALI_MIN_RANGE_X_Y		-3680 /*-230*16*/
#define CALI_MAX_RANGE_X_Y	3680 /*230*16*/
#define CALI_MIN_RANGE_Z		12704 /* 1024*16+230*16*/
#define CALI_MAX_RANGE_Z		20064 /* 1024*16-230*16*/
...
static int n2dm_acc_calibrate(struct sensors_classdev *sensors_cdev,
	...
	average[0] = arry[0] / ACC_CALI_COUNT;
	average[1] = arry[1] / ACC_CALI_COUNT;
	average[2] = arry[2] / ACC_CALI_COUNT;
	...
	if (turned_over == 1) {
		if (average[0] > CALI_MAX_RANGE_X_Y || average[0] < CALI_MIN_RANGE_X_Y ||
			average[1] > CALI_MAX_RANGE_X_Y || average[1] < CALI_MIN_RANGE_X_Y ||
			average[2] > -CALI_MIN_RANGE_Z || average[2] < -CALI_MAX_RANGE_Z) {
			pr_info("%s failed %d %d %d! Please keep the phone stable and horiztal!\n", __func__,
				average[0], average[2], average[2]);
			stat->cali_data[0] = 0;
			stat->cali_data[1] = 0;
			stat->cali_data[2] = 0;
			memset(stat->calibrate_buf, 0, sizeof(stat->calibrate_buf));
		...
		if (average[0] > CALI_MAX_RANGE_X_Y || average[0] < CALI_MIN_RANGE_X_Y ||
			average[1] > CALI_MAX_RANGE_X_Y || average[1] < CALI_MIN_RANGE_X_Y ||
			average[2] > CALI_MAX_RANGE_Z || average[2] < CALI_MIN_RANGE_Z) {
			pr_info("%s hh ==failed %d %d %d! Please keep the phone stable and horiztal!\n", __func__,
				average[0], average[1], average[2]);
			stat->cali_data[0] = 0;
			stat->cali_data[1] = 0;
			stat->cali_data[2] = 0;
...
#endif
	stat->cali_data[0] = average[0];
	stat->cali_data[1] = average[1];
#ifdef ACC_CAL_FOR_PV
	if (turned_over == 1)
		stat->cali_data[2] = average[2] + 16384; /* 1024*16=16384*/
	else
#endif
		stat->cali_data[2] = average[2] - 16384; /* 1024*16=16384*/

	#ifdef DEBUG_CALI
	pr_info("######acc cali result is %d,%d,%d\n", stat->cali_data[0], stat->cali_data[1], stat->cali_data[2]);
	#endif
	snprintf(stat->calibrate_buf, sizeof(stat->calibrate_buf), "%d,%d,%d",
	stat->cali_data[0], stat->cali_data[1], stat->cali_data[2]);
	sensors_cdev->params = stat->calibrate_buf;
...
static int n2dm_acc_write_calibrate(struct sensors_classdev *sensors_cdev, struct cal_result_t *cal_result)
{
	...
	stat->cali_data[2] = cal_result->offset_z;
      #ifdef DEBUG_CALI
	pr_info("*****acc write result is %d,%d,%d\n", stat->cali_data[0], stat->cali_data[1], stat->cali_data[2]);
      #endif
	return 0;
}
		...
		const struct i2c_device_id *id)
{
	...
	pr_info("ACC cal(): start!!!!!!!\n");
	stat->cdev.sensors_write_cal_params = n2dm_acc_write_calibrate;
